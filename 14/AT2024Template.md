...
## Производительность
### **Время отклика**: 
При выполнении основных пользовательских операций (загрузка списков, открытие карточек, сохранение данных) среднее время отклика системы — до 2 секунд, максимальное — до 5 секунд при пиковой нагрузке.
Формирование отчётов (PDF, Excel) — до 30 секунд при объёме до 10 000 строк.
### **Пропускная способность**:
Система должна поддерживать работу до 100 одновременных пользователей и не менее 50 запросов в секунду без деградации производительности.
### **Утилизация ресурсов**: 
При номинальной нагрузке использование CPU не более 70 %, оперативной памяти — не более 75 %.
Клиентское приложение (в браузере) не должно потреблять более 500 МБ ОЗУ.
### **Масштабируемость**:
Архитектура должна поддерживать горизонтальное масштабирование серверов и кэширование часто используемых данных для уменьшения нагрузки на СУБД.

## Ремонтопригодность
### **Обновление и сопровождение**:
Обновления должны выполняться без длительных простоев.
Перезапуск или восстановление системы после сбоя - до 30 минут.

### **Тестирование и CI/CD**:
Ключевые модули должны быть покрыты тестами не менее чем на 70 %.
Развёртывание новой версии осуществляется через автоматизированный конвейер CI/CD.

### **Логирование и мониторинг**:
Система ведёт централизованные логи ошибок и операций и имеет инструменты мониторинга состояния сервисов.

### **Резервное копирование**:
Бэкап БД - ежедневно, хранение копий не менее 30 дней.
Восстановление данных из резервной копии - до 1 часа.

### **Документация**:
Должна поддерживаться техническая и эксплуатационная документация, обновляемая при изменении архитектуры и релизах.

...
| 2025-29-11   | 0.6    | Шестая ревизия - итоговая. Добавление различных ролей после интервью 29.11.2025 | Баданов, Власова, Зязева, Романов, Шакирова, Якимов |
...
#### 3.1.4. Требования для роли «Коммерческий клиент»

| ID          | Наименование                                   | Важность | Стабильность | Описание |
|-------------|-------------------------------------------------|----------|--------------|----------|
| **F-CL-001** | Создание и редактирование шаблона рассылки     | M        | S            | Клиент может создавать и редактировать шаблоны коммерческих рассылок, задавая название и текст сообщения. |
| **F-CL-002** | Настройка целевой аудитории по демографии       | M        | S            | Фильтрация аудитории по возрасту, полу и другим демографическим признакам. |
| **F-CL-003** | Настройка целевой аудитории по геолокации       | M        | S            | Задание зоны рассылки на карте (радиус, полигон) на основе места жительства или текущего местоположения абонентов. |
| **F-CL-004** | Настройка аудитории по интересам и поведению    | M        | M            | Выбор целевой аудитории на основе категорий интересов и истории взаимодействия с предыдущими рассылками. |
| **F-CL-005** | Просмотр предварительного охвата аудитории      | M        | S            | Расчет и отображение оценочного количества абонентов, соответствующих критериям рассылки, до ее запуска. |
| **F-CL-006** | Просмотр и подтверждение сметы рассылки         | M        | S            | Получение предварительного счета на основе охвата аудитории и тарифа. Подтверждение сметы для запуска рассылки. |
| **F-CL-007** | Запуск и оплата рассылки                        | M        | S            | Инициирование массовой рассылки после подтверждения и проведения оплаты (вручную или через интегрированную платежную систему). |
| **F-CL-008** | Управление доступом сотрудников                 | S        | M            | Возможность создавать учетные записи для сотрудников компании с назначением прав (например, «Редактор», «Подписант», «Аналитик»). |
| **F-CL-009** | Просмотр истории и отчетов по кампаниям         | S        | S            | Доступ к детальной истории всех рассылок с данными об охвате, стоимости и базовых показателях эффективности (доставляемость, кликабельность). |


#### 3.1.5. Требования для роли «Конечный абонент»

| ID          | Наименование                                | Важность | Стабильность | Описание |
|-------------|----------------------------------------------|----------|--------------|----------|
| **F-SB-001** | Корректное определение демографии            | M        | S            | Система должна точно определять и хранить демографические данные абонента (пол, возраст), полученные от оператора связи или из иных доверенных источников. |
| **F-SB-002** | Корректное определение географического местоположения | M | S | Система должна определять постоянное место жительства и/или актуальное местоположение абонента на основе данных сотовых вышек для целевой доставки сообщений. |
| **F-SB-003** | Учет профессионального статуса               | M        | S            | Система должна корректно отражать принадлежность абонента к специальным группам (медики, военнообязанные и т.д.) на основе актуальных официальных данных. |
| **F-SB-004** | Учет предпочтений и истории взаимодействия   | O        | M            | Система должна собирать и обрабатывать агрегированные данные о реакции абонента на коммерческие рассылки (отписка, переход по ссылке) для формирования поведенческого профиля. |
| **F-SB-005** | Гарантированная доставка экстренных оповещений | M      | S            | Система должна обеспечивать приоритет и гарантированную доставку сообщений от государственных экстренных служб, независимо от настроек коммерческих рассылок. |


#### 3.1.6. Требования для роли «Служба поддержки и аналитики»

| ID          | Наименование                         | Важность | Стабильность | Описание |
|-------------|--------------------------------------|----------|--------------|----------|
| **F-SP-001** | Доступ к данным клиентского аккаунта | M        | S            | Возможность просмотра учетных записей коммерческих клиентов, истории их действий и созданных рассылок для диагностики проблем. |
| **F-SP-002** | Просмотр детальных логов рассылок    | M        | S            | Доступ к детализированной информации по конкретной рассылке: статус доставки для каждого абонента, ошибки, полученные от операторов связи. |
| **F-SP-003** | Ручная коррекция рассылки            | S        | M            | Возможность приостановить, возобновить или отменить запущенную рассылку по запросу клиента или при обнаружении критической ошибки. |
| **F-SP-004** | Доступ к агрегированной аналитике    | M        | M            | Доступ к инструментам анализа эффективности кампаний по различным срезам: клиент, период, тип аудитории, геолокация. |
| **F-SP-005** | Формирование аналитических отчетов   | S        | M            | Возможность создавать и экспортировать кастомные отчеты для клиентов с ключевыми метриками (CTR, конверсия, охват) и рекомендациями. |
| **F-SP-006** | Мониторинг удовлетворенности клиентов | S        | M            | Доступ к метрикам и отзывам от клиентов для оценки качества сервиса и выявления областей для улучшения. |



#### 3.1.7. Системные функциональные требования
